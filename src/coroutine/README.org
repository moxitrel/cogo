...
- co_begin (...)    :: Set coroutine begin.
                       List with the line numbers of *co_return*, *co_call*, *co_sched*, *co_wait*, *co_broadcast*,
                       or omit if enable GNUC extension.
- co_end   ()       :: Set coroutine end.
- co_return()       :: Yield
- co_call  (co_t &) :: Await a coroutine to finish.
- co_sched (co_t &) :: Add   a coroutine to current scheduler.
...
- o.run()   :: Run until all coroutines finished.
+ o.state() :: Return current running state.
  -  0: ready.
  - >0: running.
