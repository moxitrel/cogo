...
static void test_yield(void) {
  ...
  TEST_ASSERT_EQUAL_UINT(CO_STATUS_INITED, co_status(&co));  // init
  TEST_ASSERT_EQUAL_INT(0, co.v);

  coyield_func(&co);
  TEST_ASSERT_NOT_EQUAL_UINT(CO_STATUS_INITED, co_status(&co));   // running
  TEST_ASSERT_NOT_EQUAL_UINT(CO_STATUS_STOPPED, co_status(&co));  // running
  TEST_ASSERT_EQUAL_INT(1, co.v);

  coyield_func(&co);
  TEST_ASSERT_EQUAL_UINT(CO_STATUS_STOPPED, co_status(&co));  // end
  TEST_ASSERT_EQUAL_INT(2, co.v);

  // noop when coroutine end
  coyield_func(&co);
  TEST_ASSERT_EQUAL_UINT(CO_STATUS_STOPPED, co_status(&co));  // running
  TEST_ASSERT_EQUAL_INT(2, co.v);
}
...
static void test_prologue(void) {
  ...
  while (co_status(&co) != CO_STATUS_STOPPED) {
    prologue_func(&co);
  }
