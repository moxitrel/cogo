...
#
# cscope.files: 需要索引的 所有文件列表, 通常使用 find 来生产
#
...
CSCOPE_FILE=""              # 默认 cscope.files
OUT_FILE="../cscope.out"    # 默认 cscope.out, cscope.in.out, cscope.po.out
INCLUDE_DIR_S=("")          # 额外搜索的头文件 目录列表
...
cmd="$cmd ${INCLUDE_DIR_S:+ ${INCLUDE_DIR_S[*]/#/-I }} " # -I, 额外在 $INCLUDE_DIR 目录下搜索 头文件，可多个-I叠加
cmd="$cmd ${SRC_DIR:+       -s $SRC_DIR} "               # -s, 额外在 $SRC_DIR     目录下搜索 源文件，被忽略 若命令行已指定额外的文件
cmd="$cmd ${CSCOPE_FILE:+   -i $CSCOPE_FILE} "           # -i, 索引文件, 默认 cscope.files; 空格, tab 或 换行 分隔,
