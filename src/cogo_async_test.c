...
static void test_nat(void) {
  nat_t n = CO_INITER(&n, /*TYPE*/ nat_t);  // "v" isn't explicitly initialized
...
static void test_chan(void) {
  ...
  recv_t r1 = CO_INITER(&r1, recv_t, &chan);
  send_t s1 = CO_INITER(&s1, send_t, &chan);
  main_t m1 = CO_INITER(&m1, main_t, &r1, &s1);
  CO_RUN(&m1);
